...
func findDuplicate(paths []string) [][]string {
	...
	for _, path := range paths {
		...
		dir := arr[0]
		for i := 1; i < len(arr); i++ {
			name, ct := splitNameContent(arr[i])
			content[ct] = append(content[ct], dir+"/"+name)
		}
	}
...
func splitNameContent(file string) (name, content string) {
	name = strings.Split(file, "(")[0]
	content = strings.Split(strings.Split(file, "(")[1], ")")[0]
	return
}
