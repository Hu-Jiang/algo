...
func smallestSufficientTeam(req_skills []string, people [][]string) []int {
	...
	for i := 0; i < len(req_skills); i++ {
		skillsBitmap[req_skills[i]] = i
	}

	dp := make([][]int, 1<<uint(len(req_skills))) // all skills combination
	dp[0] = []int{}

	for i := 0; i < len(people); i++ {
		pskills := 0
		for j := 0; j < len(people[i]); j++ {
			pskills |= (1 << uint(skillsBitmap[people[i][j]]))
		}
		for k := 0; k < len(dp); k++ {
			if dp[k] != nil && pskills != 0 {
				teamSkills := pskills | k
				if len(dp[teamSkills]) == 0 || len(dp[teamSkills]) > len(dp[k])+1 {
					dp[teamSkills] = append(append([]int{}, dp[k]...), i)
				}
			}
		}
	}
